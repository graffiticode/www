...
export default async function LanguagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <Container className="py-16">
      <Link href="/languages" className="text-sm text-zinc-400 transition hover:text-white">
        ← All tools
      </Link>

      <div className="mt-4 flex items-baseline justify-between gap-4">
        <h1 className="text-3xl font-semibold tracking-tight text-white">{lang.name}</h1>
        <span className="font-mono text-sm text-zinc-500">{lang.id}</span>
      </div>
      <p className="mt-3 max-w-2xl text-lg text-zinc-300">{lang.blurb}</p>

      <div className="mt-8 grid gap-8 lg:grid-cols-2 lg:items-start">
        <div>
          <h2 className="text-sm font-medium text-zinc-400">Live example</h2>
          <Embed className="mt-2" itemId={lang.showcaseItemId} title={`${lang.name} (${lang.id})`} height={400} />
        </div>

        <div>
          <h2 className="text-sm font-medium text-zinc-400">Make one like it</h2>
          ...
          <div className="mt-4 rounded-lg border border-white/10 bg-zinc-900/50 p-4 text-sm text-zinc-400">
            <div className="font-mono text-xs text-zinc-500">item types</div>
            <div className="mt-1 flex flex-wrap gap-1.5">
              {lang.itemTypes.map((t) => (
                <span key={t} className="rounded bg-white/5 px-2 py-0.5 font-mono text-[11px] text-zinc-300">
                  {t}
                </span>
              ))}
            </div>
